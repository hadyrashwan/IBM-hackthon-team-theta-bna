...
function transferPackage(object) {
    ...
    packageSent.transporter.push(object.transporter.getIdentfier())
    packageSent.expectedToReceive=object.receiver.getIdentfier()

    if(parseInt(object.weight) > parseInt(packageSent.weight)*1.05 && parseInt(object.weight) < parseInt(packageSent.weight)*0.95 ){
        throw new Error
    }
...
function approveTransaction(object) {
    ...
    packageSent.lastSeenTimeStamp = new Date().getTime()
    packageSent.location = object.destination
    packageSent.transporter=object.transporter.getIdentfier();
